...
export async function fetchBoard(): Promise<BoardData> {
  /**
   * Fetches board columns, tasks, and team members.
...
export async function createTask(partial: Partial<Task>): Promise<Task | null> {
  /** Creates a task with provided fields. */
  if (!isSupabaseConfigured()) return null;
  const { data, error } = await supabase.from("tasks").insert(partial).select("*").single();
